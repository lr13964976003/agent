...
* A **2-layer Dense Transformer model**.


The batch size is fixed at 1024, the sequence length is fixed at 10000 , num of heads is fixed at 16, dimension of each head is fixed at 512, hidden size of MLP is fixed at 32768 for all tests.
...
Our experiments on 16 NVIDIA H100 GPUs with dense transformer model demonstrated that the proposed method achieves substantial improvements in inference throughput (up to 35%) while reducing communication overhead by over 30%, compared to a strong baseline using tensor and pipeline parallelism.
