...
def analyze_dag(dot_content):
    ...
    node_inputs = {node: set() for node in nodes}
    node_outputs = {node: set() for node in nodes}
    ...
    # Check 5: Cycle detection using DFS
    print("\n=== CYCLE DETECTION ===")
    visited = set()
    
    def has_cycle_util(node, rec_stack):
        ...
        for neighbor in neighbors:
            if has_cycle_util(neighbor, rec_stack.copy()):
                return True
