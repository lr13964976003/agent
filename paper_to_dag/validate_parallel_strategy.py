...
def validate_parallel_strategy(strategy_file):
    ...
    for gpu_name, workload in compute_dist.items():
        ...
        # Parse GPU number
        gpu_num = int(gpu_name.split('_')[1])
    ...
    memory_config = strategy['load_balancing']['memory_balance']
    max_memory = memory_config['max_memory_per_gpu']
    ...
    for metric_name, metric_value in metrics.items():
        ...
        if 'gpu_utilization' in metric_name:
            if '95%+' in metric_value:
                warnings.append("GPU utilization claim of 95%+ may be optimistic for this configuration")
    ...
    if len(strategy['stages']) >= 2:
        ...
        embed_devices = set(strategy['stages'][0]['tensor_splits']['embed_1']['devices'])
        expert_devices = set(strategy['stages'][1]['tensor_splits']['expert_2']['devices'])
    ...
    if agg_stage['parallel_mode'] == 'data_parallel' and agg_stage['replicas'] == 1:
        warnings.append("Aggregation stage uses data_parallel with only 1 replica - consider tensor_parallel for better utilization")
...
if __name__ == "__main__":
    strategy_file = "../outputs/2025-12-01-11-42-14/optimized_parallel_strategy.json"
