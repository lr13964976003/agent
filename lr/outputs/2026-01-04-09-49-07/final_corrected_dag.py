...
def generate_final_corrected_moe_dag():
    """Generate final corrected MoE DAG with all issues fixed"""
    
    dot_content = """// Qwen3-235B MoE Parallel Strategy DAG - Final Corrected Version
digraph {
    rankdir=TB
    size="20,30"
...
'''
    ...
    # Connect experts properly with both incoming and outgoing edges
    for gpu_id in range(8):
        # Connect send to experts
        for expert_id in range(4):
            dot_content += f'    comm_expert_send_gpu{gpu_id}_layer1 -> expert_gpu{gpu_id}_exp{expert_id}_layer1\n'
        
        # Connect experts to receive (this was missing in the original)
        for expert_id in range(4):
            dot_content += f'    expert_gpu{gpu_id}_exp{expert_id}_layer1 -> comm_expert_recv_gpu{gpu_id}_layer1\n'

    # Gate routing connections (dashed lines) - FIXED: reference correct expert nodes
    for src_gpu in range(8):
        for dst_gpu in range(8):
            # Reference the first expert on each GPU (exp0) for routing demonstration
...
def main():
    ...
    dot_content = generate_final_corrected_moe_dag()
    
    # Save to file
    output_path = './outputs/2026-01-04-09-49-07/moe_parallel_dag_final.dot'
    with open(output_path, 'w') as f:
        f.write(dot_content)
    
    print(f"Final corrected DAG generated and saved to {output_path}")
    
    # Generate SVG image
    svg_path = './outputs/2026-01-04-09-49-07/moe_parallel_dag_final.svg'
