...
total_before_overhead = param_per_gpu + kv_per_gpu + activation_per_gpu
overhead_amount = total_before_overhead * 0.05
final_total = total_before_overhead + overhead_amount

print(f"Subtotal per GPU: {total_before_overhead/1e9:.2f} GB")
print(f"Overhead per GPU: {overhead_amount/1e9:.2f} GB")
print(f"Final total per GPU: {final_total/1e9:.2f} GB")
print(f"Utilization: {final_total/gpu_memory*100:.1f}%")
print()
...
print("=== SUMMARY ===")
print(f"Memory utilization: {final_total/gpu_memory*100:.1f}% (target <80%)")
