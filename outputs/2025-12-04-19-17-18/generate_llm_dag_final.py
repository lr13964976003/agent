...
def create_llm_dag():
    ...
    total_gpus = pp_size * ep_size * tp_size  # 128 GPUs
    
    # Create input node
    input_node = 'input'
    gpu_range = f"0-{total_gpus-1}"
    input_label = f'Input Layer\\nGPU: {gpu_range}\\nInput: [batch_size={batch_size}, seq_len={sequence_length}, hidden={token_dimension}]\\nOutput: [batch_size={batch_size}, seq_len={sequence_length}, hidden={token_dimension}]'
    dot.node(input_node, input_label, shape='rectangle', style='filled', fillcolor='lightblue')
    ...
    for pp_stage in range(pp_size):
        ...
        for layer_idx in range(start_layer, end_layer):
            ...
            for tp_rank in range(tp_size):
                ...
                qkv_label = f'QKV Linear\\nGPU: {gpu_id}\\nInput: [batch={batch_size}, seq={sequence_length}, hidden={token_dimension}]\\nOutput: [batch={batch_size}, seq={sequence_length}, heads=32, d_k={token_dimension//32}]'
                dot.node(qkv_node, qkv_label, shape='rectangle', style='filled', fillcolor='lightblue')
            
            # Attention computation
            for tp_rank in range(tp_size):
                gpu_id = pp_stage * (ep_size * tp_size) + tp_rank
                att_comp_node = f'attention_{layer_idx}_tp{tp_rank}'
                att_label = f'Attention Computation\\nGPU: {gpu_id}\\nInput: [batch={batch_size}, seq={sequence_length}, heads=32, d_k={token_dimension//32}]\\nOutput: [batch={batch_size}, seq={sequence_length}, heads=32, d_k={token_dimension//32}]'
                dot.node(att_comp_node, att_label, shape='rectangle', style='filled', fillcolor='lightblue')
            
            # Output Linear (Row Parallel)
            for tp_rank in range(tp_size):
                gpu_id = pp_stage * (ep_size * tp_size) + tp_rank
                out_linear_node = f'att_out_linear_{layer_idx}_tp{tp_rank}'
                out_label = f'Attention Output Linear\\nGPU: {gpu_id}\\nInput: [batch={batch_size}, seq={sequence_length}, heads=32, d_k={token_dimension//32}]\\nOutput: [batch={batch_size}, seq={sequence_length}, hidden={token_dimension}]'
                dot.node(out_linear_node, out_label, shape='rectangle', style='filled', fillcolor='lightblue')
            
            # All-reduce for attention output
            allreduce_att_node = f'allreduce_att_{layer_idx}'
            gpu_range = f"{pp_stage*ep_size*tp_size}-{(pp_stage+1)*ep_size*tp_size-1}"
            allreduce_label = f'All-Reduce Attention\\nGPU: {gpu_range}\\nInput: [batch={batch_size}, seq={sequence_length}, hidden={token_dimension}]\\nOutput: [batch={batch_size}, seq={sequence_length}, hidden={token_dimension}]'
            dot.node(allreduce_att_node, allreduce_label, shape='ellipse', style='filled', fillcolor='lightgreen')
            ...
            for ep_rank in range(ep_size):
                for expert_idx in range(experts_per_ep):
                    ...
                    expert_label = f'Expert {ep_rank*experts_per_ep + expert_idx}\\nGPU: {expert_gpu}\\nInput: [batch={batch_size//8}, seq={sequence_length}, hidden={token_dimension}]\\nOutput: [batch={batch_size//8}, seq={sequence_length}, hidden={moe_hidden_size}]'
                    dot.node(expert_node, expert_label, shape='rectangle', style='filled', fillcolor='lightblue')
            ...
            for ep_rank in range(ep_size):
                ...
                gpu_range = f"{start_gpu}-{end_gpu}"
                route_label = f'Token Routing EP{ep_rank}\\nGPU: {gpu_range}\\nInput: [batch={batch_size}, seq={sequence_length}, hidden={token_dimension}]\\nOutput: [batch={batch_size//8}, seq={sequence_length}, hidden={token_dimension}]'
                dot.node(route_node, route_label, shape='ellipse', style='filled', fillcolor='lightgreen')
            ...
            for ep_rank in range(ep_size):
                for expert_idx in range(experts_per_ep):
                    ...
                    expert_gpu = pp_stage * (ep_size * tp_size) + ep_rank * tp_size
                    comp_label = f'Expert FFN {ep_rank*experts_per_ep + expert_idx}\\nGPU: {expert_gpu}\\nInput: [batch={batch_size//8}, seq={sequence_length}, hidden={token_dimension}]\\nOutput: [batch={batch_size//8}, seq={sequence_length}, hidden={moe_hidden_size}]'
                    dot.node(expert_comp_node, comp_label, shape='rectangle', style='filled', fillcolor='lightblue')
            
            # Expert output aggregation
            expert_agg_node = f'expert_agg_{layer_idx}'
            gpu_range = f"{pp_stage*ep_size*tp_size}-{(pp_stage+1)*ep_size*tp_size-1}"
            agg_label = f'Expert Output Aggregation\\nGPU: {gpu_range}\\nInput: [batch={batch_size//8}, seq={sequence_length}, hidden={moe_hidden_size}]\\nOutput: [batch={batch_size}, seq={sequence_length}, hidden={moe_hidden_size}]'
            dot.node(expert_agg_node, agg_label, shape='parallelogram', style='filled', fillcolor='lightyellow')
            
            # Final MoE linear (Tensor Parallel)
            for tp_rank in range(tp_size):
                gpu_id = pp_stage * (ep_size * tp_size) + tp_rank
                moe_linear_node = f'moe_out_linear_{layer_idx}_tp{tp_rank}'
                moe_label = f'MoE Output Linear\\nGPU: {gpu_id}\\nInput: [batch={batch_size}, seq={sequence_length}, hidden={moe_hidden_size}]\\nOutput: [batch={batch_size}, seq={sequence_length}, hidden={token_dimension}]'
                dot.node(moe_linear_node, moe_label, shape='rectangle', style='filled', fillcolor='lightblue')
            
            # All-reduce for MoE output
            allreduce_moe_node = f'allreduce_moe_{layer_idx}'
            gpu_range = f"{pp_stage*ep_size*tp_size}-{(pp_stage+1)*ep_size*tp_size-1}"
            allreduce_moe_label = f'All-Reduce MoE\\nGPU: {gpu_range}\\nInput: [batch={batch_size}, seq={sequence_length}, hidden={token_dimension}]\\nOutput: [batch={batch_size}, seq={sequence_length}, hidden={token_dimension}]'
            dot.node(allreduce_moe_node, allreduce_moe_label, shape='ellipse', style='filled', fillcolor='lightgreen')
            
            # Layer normalization
            layernorm_node = f'layernorm_{layer_idx}'
            gpu_range = f"{pp_stage*ep_size*tp_size}-{(pp_stage+1)*ep_size*tp_size-1}"
            layernorm_label = f'Layer Normalization\\nGPU: {gpu_range}\\nInput: [batch={batch_size}, seq={sequence_length}, hidden={token_dimension}]\\nOutput: [batch={batch_size}, seq={sequence_length}, hidden={token_dimension}]'
            dot.node(layernorm_node, layernorm_label, shape='rectangle', style='filled', fillcolor='lightblue')
    
    # Create output node
    output_node = 'output'
    gpu_range = f"0-{total_gpus-1}"
    output_label = f'Output Layer\\nGPU: {gpu_range}\\nInput: [batch_size={batch_size}, seq_len={sequence_length}, hidden={token_dimension}]\\nOutput: [batch_size={batch_size}, seq_len={sequence_length}, vocab_size=50000]'
    dot.node(output_node, output_label, shape='rectangle', style='filled', fillcolor='lightblue')
