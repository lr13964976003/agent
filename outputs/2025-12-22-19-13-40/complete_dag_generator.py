...
digraph OptimalStrategy {
    ...
    subgraph cluster_stage3 {
        ...
        subgraph cluster_layer12_attn {
            ...
            l12_attn_out [label="L12-Attention-失重\\nTP4[384-387]\\nIn: [16,1024,1024]\\nOut: [16,1024,1024]", shape=rectangle, fillcolor=lightblue, style=filled];
...
def main():
    """Generate both DAG files"""
    
    # Create current strategy DAG
    current_dot = create_current_strategy_dag()
    current_file = "../outputs/2025-12-22-19-13-40/current_strategy.dot"
    
    with open(current_file, 'w') as f:
        f.write(current_dot)
    
    # Create optimal strategy DAG
    optimal_dot = create_optimal_strategy_dag()
    optimal_file = "../outputs/2025-12-22-19-13-40/optimal_strategy.dot"
    
    with open(optimal_file, 'w') as f:
        f.write(optimal_dot)
    
    print(f"Generated DAG files:")
    print(f"  Current Strategy: {current_file}")
    print(f"  Optimal Strategy: {optimal_file}")
    
    # Generate SVG images using graphviz
    try:
        subprocess.run(['dot', '-Tsvg', current_file, '-o', '../outputs/2025-12-22-19-13-40/current_strategy.svg'], check=True)
        subprocess.run(['dot', '-Tsvg', optimal_file, '-o', '../outputs/2025-12-22-19-13-40/optimal_strategy.svg'], check=True)
    ...
    return {
        "current_strategy_dot": current_file,
        "optimal_strategy_dot": optimal_file,
        "current_strategy_svg": "../outputs/2025-12-22-19-13-40/current_strategy.svg",
        "optimal_strategy_svg": "../outputs/2025-12-22-19-13-40/optimal_strategy.svg"
    }
