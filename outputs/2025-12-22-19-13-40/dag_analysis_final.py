...
def analyze_parallel_strategy(G: nx.DiGraph, strategy_name: str, content: str) -> List[str]:
    ...
    if strategy_match:
        ...
        if tp_match:
            ...
            # Count TP nodes - look for attention nodes that should be TP-decomposed
            attention_nodes = [n for n in G.nodes() if 'attn' in n.lower() or 'attention' in n.lower()]
            if len(attention_nodes) < 4:  # Should have at least QKV, Score, Output, AllReduce for attention
        ...
        if ep_match:
            ...
            # Count expert nodes
            expert_nodes = [n for n in G.nodes() if 'expert' in n.lower()]
            if len(expert_nodes) < ep_size // 8:  # Should have reasonable number of expert groups
...
def check_gpu_communications(G: nx.DiGraph, content: str) -> List[str]:
    ...
    communication_keywords = ['allreduce', 'dispatch', 'combine', 'alltoall']
    communication_nodes = []
...
def check_attention_decomposition(G: nx.DiGraph, content: str) -> List[str]:
    ...
    if not attention_nodes:
        issues.append("No attention nodes found")
        return issues
    
    # Check if we have the basic attention pipeline components
    qkv_nodes = [n for n in attention_nodes if 'qkv' in n.lower()]
    score_nodes = [n for n in attention_nodes if 'score' in n.lower()]
    output_nodes = [n for n in attention_nodes if 'out' in n.lower()]  # Changed from 'output' to 'out'
    allreduce_nodes = [n for n in attention_nodes if 'allreduce' in n.lower()]
    ...
    if not allreduce_nodes:
        issues.append("Missing allreduce communication in attention")
