...
def generate_complete_moe_dag():
    """Generate a complete MoE model DAG with TP×EP×PP=2×2×2 parallelism"""
    
    dot_content = '''digraph MoE_Complete_DAG {
    rankdir=TB;
...
def main():
    ...
    dot_content = generate_complete_moe_dag()
    
    # Save to file
    output_dir = "../outputs/2025-12-24-15-21-58"
    dot_file = os.path.join(output_dir, "moe_complete_dag_final.dot")
    
    with open(dot_file, 'w') as f:
        f.write(dot_content)
    
    print(f"Generated complete DAG: {dot_file}")
    
    # Generate SVG and PNG using graphviz
    try:
        import subprocess
        svg_file = os.path.join(output_dir, "moe_complete_dag_final.svg")
        png_file = os.path.join(output_dir, "moe_complete_dag_final.png")
        ...
        print("Please install graphviz to generate visualizations")
    
    return {
        "dot_file": dot_file,
        "svg_file": svg_file if 'svg_file' in locals() else None,
        "png_file": png_file if 'png_file' in locals() else None
    }
