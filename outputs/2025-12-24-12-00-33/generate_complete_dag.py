...
def generate_complete_dag():
    """Generate complete LLM parallel strategy deployment DAG with all connections fixed"""
    
    dot_content = '''// Complete LLM Parallel Strategy Deployment DAG - Fixed Version
digraph {
...
'''
    ...
    # Write the complete DAG to file
    dot_file_path = "../outputs/2025-12-24-12-00-33/llm_parallel_deployment_fixed.dot"
    with open(dot_file_path, 'w') as f:
        f.write(dot_content)
    
    print(f"Complete DAG written to: {dot_file_path}")
    
    # Generate SVG using graphviz
    try:
        import subprocess
        svg_file_path = "../outputs/2025-12-24-12-00-33/llm_parallel_deployment_fixed.svg"
        subprocess.run(['dot', '-Tsvg', dot_file_path, '-o', svg_file_path], check=True)
        print(f"SVG image generated: {svg_file_path}")
...
if __name__ == "__main__":
    dot_path, svg_path = generate_complete_dag()
