...
def create_baseline_dag():
    ...
    for stage in [0, 1]:
        ...
        # Add stage boundary (cluster)
        with dot.subgraph(name=f'cluster_stage_{stage}') as stage_sub:
            stage_sub.attr(label=f'Pipeline Stage {stage}\nGPUs {stage_start}-{stage_start+7}')
            stage_sub.attr(style='dashed', color='black')
            ...
            for layer in range(8):
                ...
                for tp_shard in range(8):
                    ...
                    stage_sub.node(attn_node, 
                                 f'Attention Layer {actual_layer}\nTP Shard {tp_shard}\nGPU {gpu_id}\nInput: [128,10000,4,128], Output: [128,10000,4,128]',
                ...
                for tp_shard in range(8):
                    ...
                    stage_sub.node(moe_node,
                                 f'MoE Layer {actual_layer}\nExperts {expert_start}-{expert_end}\nGPU {gpu_id}\nInput: [128,10000,4096], Output: [128,10000,4096]',
...
def create_proposed_dag():
    ...
    # Process layers - focusing on representative layers due to 16*16=256 GPUs
    ...
    for layer_idx in representative_layers:
        ...
        for expert_id in range(16):
            expert_gpu = layer_idx * 16 + expert_id + 1  # GPUs 1-16, 17-32, etc.
            expert_node = f"{layer_prefix}_expert{expert_id}_gpu{expert_gpu}"
            dot.node(expert_node,
                    f'Expert {expert_id} Layer {layer_idx}\nGPU {expert_gpu}\nInput: [tokens,4096], Output: [tokens,4096]',
                    fillcolor='lightyellow')
        ...
        dot.node(residual_node,
                f'Residual Add\nLayer {layer_idx}\nInput: [128,10000,4096]+[128,10000,4096], Output: [128,10000,4096]',
...
def main():
    ...
    baseline_dag.render('../outputs/2025-11-25-16-47-05/moe_baseline_tp8_pp2', format='svg', cleanup=False)
    ...
    proposed_dag.render('../outputs/2025-11-25-16-47-05/moe_proposed_ep16_one_expert_per_gpu', format='svg', cleanup=False)
