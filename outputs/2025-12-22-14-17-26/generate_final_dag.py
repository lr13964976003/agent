#!/usr/bin/env python3
"""
Final DAG Generator for LLM EP64-TP8-PP2-DP2 Deployment
This version has exactly 16 All-Reduce communications (1 per layer).
"""
...
def create_final_llm_dag():
    """Create the final LLM deployment DAG with correct All-Reduce count."""
    
    # Create the main DAG
    dot = Digraph(comment='LLM EP64-TP8-PP2-DP2 Deployment DAG - Final')
    ...
    # All-Reduce operations: exactly 16 total (1 per per layer)
    # Distribution: 1 All-Reduce per layer after the main TP operations
    ...
    for layer in range(8):
        ...
        # MoE Output Projection (TP8) - All-Reduce after this (1 per layer)
        dot.node(f'{layer_prefix}_moe_out', 
                 f'MoE Output{layer}\\nInput: [128,1024,2048]\\nOutput: [128,1024,1024]\\nGPU: {stage1_gpus}', 
                 **compute_style)
        
        # All-Reduce after MoE (1 per layer, 8 total for stage 1)
        dot.node(f'{layer_prefix}_ar', 
                 f'All-Reduce{layer}\\nInput: [128,1024,1024]\\nOutput: [128,1024,1024]\\nGPU: {stage1_gpus}', 
                 **comm_style)
    ...
    for layer in range(8):
        ...
        # LayerNorm1 -> QKV -> Attention -> Attention Output -> Residual
        ...
        dot.edge(f'{layer_prefix}_attn_out', f'{layer_prefix}_res1')
        ...
        dot.edge(f'{layer_prefix}_moe_out', f'{layer_prefix}_ar')
        dot.edge(f'{layer_prefix}_ar', f'{layer_prefix}_res2')
...
def main():
    """Main function to generate the final DAG."""
    
    print("Generating final LLM deployment DAG...")
    
    # Create the final DAG
    dot = create_final_llm_dag()
    ...
    if is_valid:
        # Save the DOT file
        dot_file = '../outputs/2025-12-22-14-17-26/llm_final_dag.dot'
        dot.save(dot_file)
        print(f"Saved DOT file: {dot_file}")
        
        # Render to SVG
        svg_file = '../outputs/2025-12-22-14-17-26/llm_final_dag.svg'
        dot.render('../outputs/2025-12-22-14-17-26/llm_final_dag', format='svg', cleanup=True)
        ...
        return {
            "dag_files": [
                "../outputs/2025-12-22-14-17-26/llm_final_dag.dot",
                "../outputs/2025-12-22-14-17-26/llm_final_dag.svg"
            ],
