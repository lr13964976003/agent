...
def create_corrected_proposed_layer_wise_dag():
    """
    Create corrected proposed layer-wise DAG with proper tensor parallelism
    Fixes: Added AllReduce/AllGather operations, complete communication primitives
    """
    dot = graphviz.Digraph('proposed_layer_wise_tensor_pipeline_dag', 
                          comment='Corrected Proposed Layer-wise DAG with Tensor Parallelism')
    ...
    model_specs = {
        ...
        'batch_size': 128,
        'seq_len': 10000,
        'tensor_parallel': 4,  # Added tensor parallelism
        'pipeline_parallel': 2   # Pipeline across GPUs
    }
    ...
    # Create nodes for all 16 layers with tensor parallelism
    for layer in range(16):
        # Determine GPU assignment (2 layers per GPU with tensor parallelism)
        gpu_id = layer // 2  # 0-7 for 16 layers
        
        # Layer input distribution
        dot.node(f'layer_{layer}_input_dist', f'Layer {layer}\\nInput Distribution\\nGPU {gpu_id}', 
                shape='parallelogram', fillcolor='lightyellow')
        
        # QKV Projection with tensor parallelism (4-way)
        for tp_rank in range(4):
            tp_gpu_id = gpu_id * 4 + tp_rank  # 0-31 for tensor parallelism
            dot.node(f'layer_{layer}_qkv_gpu{tp_gpu_id}', 
        ...
        for tp_rank in range(4):
            tp_gpu_id = gpu_id * 4 + tp_rank
            dot.node(f'layer_{layer}_attn_out_gpu{tp_gpu_id}', 
        ...
        for tp_rank in range(4):
            tp_gpu_id = gpu_id * 4 + tp_rank
            dot.node(f'layer_{layer}_mlp_fc1_gpu{tp_gpu_id}', 
        ...
        for tp_rank in range(4):
            tp_gpu_id = gpu_id * 4 + tp_rank
            dot.node(f'layer_{layer}_mlp_fc2_gpu{tp_gpu_id}', 
        ...
        # Inter-GPU communication for pipeline parallelism
        if layer < 15:
            next_gpu_id = (layer + 1) // 2
            if gpu_id != next_gpu_id:
                # Pipeline communication between GPUs
                dot.node(f'layer_{layer}_to_gpu{next_gpu_id}', 
                        f'Layer {layer}→{layer+1}\\nPipeline Comm\\nGPU {gpu_id}→{next_gpu_id}', 
    ...
    for layer in range(16):
        ...
        if layer == 0:
            ...
            prev_gpu_id = (layer - 1) // 2
            if gpu_id == prev_gpu_id:
                # Same GPU, direct connection
                dot.edge(f'layer_{layer-1}_ln2', f'layer_{layer}_input_dist')
            else:
                # Different GPU, pipeline communication
                dot.edge(f'layer_{layer-1}_ln2', f'layer_{layer-1}_to_gpu{gpu_id}')
                dot.edge(f'layer_{layer-1}_to_gpu{gpu_id}', f'layer_{layer}_input_dist')
        
        # Connect to QKV projections
        for tp_rank in range(4):
            tp_gpu_id = gpu_id * 4 + tp_rank
            dot.edge(f'layer_{layer}_input_dist', f'layer_{layer}_qkv_gpu{tp_gpu_id}')
        
        # Connect QKV to AllGather
        for tp_rank in range(4):
            tp_gpu_id = gpu_id * 4 + tp_rank
            dot.edge(f'layer_{layer}_qkv_gpu{tp_gpu_id}', f'layer_{layer}_allgather_qkv')
        ...
        dot.edge(f'layer_{layer}_attention', f'layer_{layer}_attn_out_gpu{gpu_id * 4}')
        
        # Connect attention outputs to AllReduce
        for tp_rank in range(4):
            tp_gpu_id = gpu_id * 4 + tp_rank
            dot.edge(f'layer_{layer}_attn_out_gpu{tp_gpu_id}', f'layer_{layer}_allreduce_attn')
        ...
        for tp_rank in range(4):
            tp_gpu_id = gpu_id * 4 + tp_rank
            dot.edge(f'layer_{layer}_ln1', f'layer_{layer}_mlp_fc1_gpu{tp_gpu_id}')
        
        # Connect MLP FC1 to AllGather
        for tp_rank in range(4):
            tp_gpu_id = gpu_id * 4 + tp_rank
            dot.edge(f'layer_{layer}_mlp_fc1_gpu{tp_gpu_id}', f'layer_{layer}_allgather_mlp')
        
        # Connect AllGather to GELU
        dot.edge(f'layer_{layer}_allgather_mlp', f'layer_{layer}_gelu')
        
        # Connect GELU to MLP FC2
        dot.edge(f'layer_{layer}_gelu', f'layer_{layer}_mlp_fc2_gpu{gpu_id * 4}')
        
        # Connect MLP FC2 to AllReduce
        for tp_rank in range(4):
            tp_gpu_id = gpu_id * 4 + tp_rank
            dot.edge(f'layer_{layer}_mlp_fc2_gpu{tp_gpu_id}', f'layer_{layer}_allreduce_mlp')
...
if __name__ == "__main__":
    dag = create_corrected_proposed_layer_wise_dag()
    
    # Save DOT file
    dot_file_path = '../outputs/2025-11-29-14-59-32/corrected_proposed_layer_wise_dag.dot'
    dag.save(dot_file_path)
    
    # Save SVG image
    svg_file_path = '../outputs/2025-11-29-14-59-32/corrected_proposed_layer_wise_dag.svg'
    dag.render(svg_file_path.replace('.svg', ''), format='svg', cleanup=True)
    
    print(f"Corrected proposed DAG generated:")
