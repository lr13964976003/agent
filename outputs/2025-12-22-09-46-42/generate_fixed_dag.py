#!/usr/bin/env python3
"""
Fixed DAG Generator for 30B MoE Model Deployment
Addresses the dp_replica node output connection issue
"""

import os

def generate_fixed_dag():
    """Generate the corrected DAG with proper DP replica connections"""
    
    dot_content = '''// 30B MoE Model Deployment DAG - Fixed Version with Proper DP Connections
digraph {
    ...
    subgraph cluster_data_parallel {
        ...
        dp_sync [label="DP Sync\\nGradient synchronization\\nAll-Reduce communication\\n2 DP replicas" fillcolor=pink shape=ellipse style="filled,dashed"]
    }
    ...
    // Fixed Data parallelism representation - now with proper output connections
    input -> dp_replica [label="DP replica" style=dotted]
    dp_replica -> dp_sync [label="gradients" style=dotted]
    dp_sync -> input_broadcast [label="synced params" style=dotted]
}'''
    
    # Write the DOT file
    dot_file_path = "../outputs/2025-12-22-09-46-42/moe_deployment_dag_fixed.dot"
...
def main():
    """Main function to generate the fixed DAG"""
    print("Generating fixed DAG for 30B MoE model deployment...")
    
    # Generate the DOT file
    dot_file_path = generate_fixed_dag()
    print(f"Generated DOT file: {dot_file_path}")
    
    # Generate SVG using graphviz
    svg_file_path = dot_file_path.replace('.dot', '.svg')
    
    try:
        import subprocess
        result = subprocess.run(['dot', '-Tsvg', dot_file_path, '-o', svg_file_path], 
        ...
        print("graphviz 'dot' command not found, skipping SVG generation")
        svg_file_path = None
    
    # Create submission JSON
    submission = {
        "dot_file": dot_file_path,
        "svg_file": svg_file_path if svg_file_path else "SVG generation skipped",
        "submission": "../outputs/2025-12-22-09-46-42/submission_fixed.json"
    }
    
    import json
    with open("../outputs/2025-12-22-09-46-42/submission_fixed.json", 'w') as f:
        json.dump(submission, f, indent=2)
    
    print("Fixed DAG generation completed!")
    return submission
