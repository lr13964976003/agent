...
def generate_llama70b_parallel_dag():
    """Generate corrected DAG for Llama3-70B with TP=2, PP=4 including missing attention gates"""
    
    dot_content = """// Llama3-70B Parallel Strategy Deployment DAG - CORRECTED
...
def main():
    ...
    dag_content = generate_llama70b_parallel_dag()
    
    # Save the DOT file
    dot_file_path = "../outputs/2025-12-25-10-57-48/llama70b_parallel_dag_corrected.dot"
    with open(dot_file_path, 'w') as f:
        f.write(dag_content)
    
    print(f"Generated corrected DOT file: {dot_file_path}")
    
    # Generate SVG using graphviz
    svg_file_path = "../outputs/2025-12-25-10-57-48/llama70b_parallel_dag_corrected.svg"
    os.system(f"dot -Tsvg {dot_file_path} -o {svg_file_path}")
    print(f"Generated corrected SVG file: {svg_file_path}")
    ...
    json_file_path = "../outputs/2025-12-25-10-57-48/dag_paths_corrected.json"
    with open(json_file_path, 'w') as f:
        json.dump(metadata, f, indent=2)
    
    print(f"Generated metadata JSON: {json_file_path}")
