...
class ParallelStrategyOptimizer:
    ...
    def calculate_memory_requirements(self) -> dict:
        ...
        # MoE layers (experts)
        # Each expert: gate + 2 FFN layers
        expert_params_per_expert = self.hidden_size + 2 * self.hidden_size * self.ffn_hidden_size
        moe_params_per_layer = self.experts_per_layer * expert_params_per_expert
        
        # Total parameters
        total_params = (embedding_params + 
                       self.layers * attention_params_per_layer + 
                       self.layers * moe_params_per_layer)
...
if __name__ == "__main__":
    ...
    with open('../outputs/2025-12-04-14-18-19/parallel_config.json', 'w') as f:
        json.dump(config_dict, f, indent=2)
    
    print("Configuration saved to parallel_config.json")