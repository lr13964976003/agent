    ...
    for gpu_idx, gpu in enumerate(gpus):
        ...
        for layer in range(num_layers):
            ...
            for expert in range(min(4, experts_per_layer)):  # Show first 4 experts
                ...
                # Dashed line from routing to expert (gate selection)
                dot.edge(f'{layer_prefix}_routing', f'{layer_prefix}_expert{expert}', 
                        style='dashed', label=f'Gate selection')
            ...
            if layer == 0:
                dot.edge(prev_node, f'{layer_prefix}_ln1')
            else:
                prev_layer = layer - 1
                dot.edge(f'{gpu_prefix}_layer{prev_layer}_res2', f'{layer_prefix}_ln1')
            ...
            dot.edge(f'{layer_prefix}_attn', f'{layer_prefix}_attn_reshape')
            dot.edge(f'{layer_prefix}_attn_reshape', f'{layer_prefix}_attn_out')
            dot.edge(f'{layer_prefix}_attn_out', f'{layer_prefix}_kv_update')
            dot.edge(f'{layer_prefix}_kv_update', f'{layer_prefix}_res1')
            
            # FFN path
            dot.edge(f'{layer_prefix}_res1', f'{layer_prefix}_ln2')
            dot.edge(f'{layer_prefix}_ln2', f'{layer_prefix}_routing')
            
            # Connect experts to aggregation
            for expert in range(min(4, experts_per_layer)):
                dot.edge(f'{layer_prefix}_expert{expert}', f'{layer_prefix}_expert_agg')
            
            dot.edge(f'{layer_prefix}_routing', f'{layer_prefix}_expert_agg')
            dot.edge(f'{layer_prefix}_expert_agg', f'{layer_prefix}_res2')
                        prev_node = f'{layer_prefix}_res2'
        
        # Final layer norm
        dot.node(f'{gpu_prefix}_final_ln',
                 f'Final LayerNorm\\nInput: [{int(batch_size/8)}, {seq_len}, {hidden_size}]\\nOutput: [{int(batch_size/8)}, {seq_len}, {hidden_size}]\\nGPU: {gpu}',
                 shape='rectangle', style='filled', fillcolor=compute_color)
        
        dot.edge(f'{gpu_prefix}_layer{num_layers-1}_res2', f'{gpu_prefix}_final_ln')
        
        # Output projection
        dot.node(f'{gpu_prefix}_output_proj',
                 f'Output Projection\\nInput: [{int(batch_size/8)}, {seq_len}, {hidden_size}]\\nOutput: [{int(batch_size/8)}, {seq_len}, {vocab_size}]\\nGPU: {gpu}',
                 shape='rectangle', style='filled', fillcolor=compute_color)
        
        dot.edge(f'{gpu_prefix}_final_ln', f'{gpu_prefix}_output_proj')
                # Softmax
        dot.node(f'{gpu_prefix}_softmax',
                 f'Softmax\\nInput: [{int(batch_size/8)}, {seq_len}, {vocab_size}]\\nOutput: [{int(batch_size/8)}, {seq_len}, {vocab_size}]\\nGPU: {gpu}',
                 shape='rectangle', style='filled', fillcolor=compute_color)
        
        dot.edge(f'{gpu_prefix}_output_proj', f'{gpu_prefix}_softmax')
    
    # Data parallel aggregation (for final outputs)
    dot.node('dp_aggregate',
             f'Data Parallel Aggregate\\nInput: [{int(batch_size/8)}, {seq_len}, {vocab_size}] x 8 GPUs\\nOutput: [{batch_size}, {seq_len}, {vocab_size}]\\nGPU: All',
             shape='parallelogram', style='filled', fillcolor=comm_color)
    
    # Connect final outputs to aggregation
    for gpu_idx, gpu in enumerate(gpus):
        dot.edge(f'gpu{gpu_idx}_softmax', 'dp_aggregate', 
                label=f'GPU {gpu_idx} output')
    
    # Final output
    dot.node('output',
             f'Final Output\\nBatch: {batch_size} sequences\\nVocab: {vocab_size}\\nGPU: All',
             shape='ellipse', style='filled', fillcolor=output_color)
    
    dot.edge('dp_aggregate', 'output', label='Combined results')
    
    # Add subgraphs for better organization
    with dot.subgraph(name='cluster_legend') as legend:
        legend.attr(label='Legend', style='dashed', color='gray')
        legend.node('legend_compute', 'Computation Node', shape='rectangle', style='filled', fillcolor=compute_color)
        legend.node('legend_comm', 'Communication Node', shape='ellipse', style='filled', fillcolor=comm_color)
        legend.node('legend_routing', 'Routing/Aggregation', shape='parallelogram', style='filled', fillcolor=routing_color)
        legend.node('legend_input', 'Input/Output', shape='ellipse', style='filled', fillcolor=input_color)
    
    return dot

def main():
    """Generate and save the DAG"""
    print("Generating LLM Deployment DAG for EP1-TP1-PP1-DP8 strategy...")
    
    # Create the DAG
    dag = create_llm_deployment_dag()
    
    # Save as DOT file
    dot_file_path = '../outputs/2025-12-22-11-27-34/llm_deployment_dag.dot'
    with open(dot_file_path, 'w') as f:
        f.write(dag.source)
    
    # Save as SVG
    svg_file_path = '../outputs/2025-12-22-11-27-34/llm_deployment_dag.svg'
    dag.render('../outputs/2025-12-22-11-27-34/llm_deployment_dag', format='svg', cleanup=True)
    
    print(f"DAG saved to:")
    print(f"  DOT file: {dot_file_path}")
    print(f"  SVG file: {svg_file_path}")
    
    # Verify the DAG has no cycles
    try:
        from Extract Info From DAG import extract_info_from_dag
        info = extract_info_from_dag(dot_file_path)
        print(f"DAG validation:")
        print(f"  Has cycles: {info['has_cycle']}")
        print(f"  Total nodes: {info['total_nodes']}")
        print(f"  Total edges: {info['total_edges']}")
    except Exception as e:
        print(f"DAG validation skipped: {e}")
