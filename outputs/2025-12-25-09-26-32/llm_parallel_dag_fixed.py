...
def create_parallel_dag():
    ...
    # Layer loop (16 layers) - simplified to show key operations
    for layer in range(16):
        ...
        for ep in range(16):
            for tp in range(4):
                ...
                dot.node(f'attn_{layer}_{ep}_{tp}', 
                        f'Attention_{layer}_{ep}_{tp}\\nGPU:{gpu_id}\\nInput: [batch_size=128, seq_len=128, heads=8, d_k=64]\\nOutput: [batch_size=128, seq_len=128, heads=BG, d_k=64]', 
                        fillcolor=gpu_colors[f'EP{ep}'])
...
if __name__ == '__main__':
    ...
    # Save as DOT file
    dag.save('../outputs/2025-12-25-09-26-32/llm_parallel_strategy_fixed.dot')
    
    # Render as SVG
    dag.render('../outputs/2025-12-25-09-26-32/llm_parallel_strategy_fixed', format='svg', cleanup=True)
    
    print("DAG generated successfully!")
    print(f"DOT file: ../outputs/2025-12-25-09-26-32/llm_parallel_strategy_fixed.dot")
    print(f"SVG file: ../outputs/2025-12-25-09-26-32/llm_parallel_strategy_fixed.svg")