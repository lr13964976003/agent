...
class MoEDAGGenerator:
    ...
    def _create_baseline_dag(self):
        ...
        # Input node
        dot.node('input', 'Model Input', shape='ellipse', fillcolor='lightgreen',
        ...
        # Output node
        dot.node('output', 'Model Output', shape='ellipse', fillcolor='lightgreen',
                label='Output\\nbatch_size=128, seq_len=10000, token_dim=4096')
    ...
    def _create_proposed_dag(self):
        ...
        for layer_idx in range(16):
            ...
            dot.node(all2all_result_name,
                    ...
                    shape='ellipse', fillcolor='yellow')
            dot.edge(agg_name, all2all_result_name)
            
            # Results to residual
            residual_name = f'{layer_name}_residual'
            dot.edge(mha_name, residual_name, label='identity')
            dot.edge(all2all_result_name, residual_name)
        ...
        for layer_idx in range(16):
            ...
            # Show explicit GPU-to-GPU communication
            for src_gpu in range(16):
                for dst_gpu in range(16):
                    if src_gpu != dst_gpu:
                        comm_name = f'{layer_name}_gpu{src_gpu}_to_gpu{dst_gpu}'
                        dot.edge(f'{layer_name}_gate', comm_name, 
                                style='dotted', label=f'GPU {src_gpu} → GPU {dst_gpu}')
