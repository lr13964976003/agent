...
def create_deepseek_r1_baseline_tp():
    dot = graphviz.Digraph('DeepSeek_R1_Baseline_TP')
    ...
    # In baseline TP, we use 4 GPUs (constrained by KV heads=1)
    gpus = [f'gpu_{i}' for i in range(4)]
    ...
    # QKV Projection (tensor parallel across 4 GPUs)
    for i, gpu in enumerate(gpus):
        start_col = i * (H // 4)
        end_col = (i + 1) * (H // 4) - 1
        dot.node(f'qkv_proj_{gpu}', f'QKV Projection\\nInput: [batch_size={B}, seq_len={S}, hidden={H}]\\nOutput: [batch_size={B}, seq_len={S}, heads={Q}, d_k={Hsz//4}]\\nDevice: {gpu}\\nColumns: {start_col}-{end_col}',
                fillcolor='lightcoral')
    
    # KV Cache (duplicated across all GPUs - this is the limitation)
    for gpu in gpus:
    ...
    for gpu in gpus:
        dot.node(f'flash_attn_{gpu}', f'FlashAttention\\nInput: [batch_size={B}, seq_len={S}, heads={Q//4}, d_k={Hsz}]\\nOutput: [batch_size={B}, seq_len={S}, heads={Q//4}, d_k={Hsz}]\\nDevice: {gpu}',
                fillcolor='lightblue')
    ...
    # Attention Output Projection (tensor parallel across 4 GPUs)
    for i, gpu in enumerate(gpus):
        start_row = i * (H // 4)
        end_row = (i + 1) * (H // 4) - 1
        dot.node(f'attn_out_{gpu}', f'Attention Output\\nInput: [batch_size={B}, seq_len={S}, hidden={H//4}]\\nOutput: [batch_size={B}, seq_len={S}, hidden={H//4}]\\nDevice: {gpu}\\nRows: {start_row}-{end_row}',
                fillcolor='lightcoral')
    ...
    # Expert FC1 - but for baseline, this is just dense FFN
    for i, gpu in enumerate(gpus):
        start_col = i * (F // 4)
        end_col = (i + 1) * (F // 4) - 1
        dot.node(f'ffn_fc1_{gpu}', f'FFN FC1\\nInput: [batch_size={B}, seq_len={S}, hidden={H//4}]\\nOutput: [batch_size={B}, seq_len={S}, ffn={F//4}]\\nDevice: {gpu}\\nColumns: {start_col}-{end_col}',
                fillcolor='lightblue')
    
    # Activation
    for gpu in gpus:
        dot.node(f'ffn_act_{gpu}', f'SwiGLU Activation\\nInput: [batch_size={B}, seq_len={S}, ffn={F//4}]\\nOutput: [batch_size={B}, seq_len={S}, ffn={F//4}]\\nDevice: {gpu}',
                fillcolor='lightblue')
...
if __name__ == '__main__':
    dag = create_deepseek_r1_baseline_tp()
    dag.render('../outputs/2025-10-29-10-09-37/deepseek_r1_baseline_tp', format='dot', cleanup=False)
    dag.render('../outputs/2025-10-29-10-09-37/deepseek_r1_baseline_tp', format='svg', cleanup=False)